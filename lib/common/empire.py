...
class MainMenu(cmd.Cmd):
    ...
    def __init__(self, args=None):
        ...
        (self.isroot, self.installPath, self.ipWhiteList, self.ipBlackList, self.obfuscate, self.obfuscateCommand) = helpers.get_config(
    ...
    def handle_event(self, signal, sender):
        ...
        try:
            ...
            print(
                helpers.color(
                    "[!] Error: bad signal recieved {} from sender {}".format(
                        signal,
                        sender)))
            return
        ...
        # TODO instead of "dispatched_event" put something useful in the
        # "event_type" column
        log_event(
            cur,
            sender,
        ...
        try:
            ...
            if os.geteuid() != 0:
                if self.isroot:
                    ...
                    while True:
                        a = input(
                            helpers.color("[>] Are you sure you want to continue (y) or (n): "))
                        if a.startswith("y"):
                            return
                        if a.startswith("n"):
                            self.shutdown()
            ...
            if self.args.listener:
                if self.args.listener == 'list':
                    ...
                    messages.display_listeners(
                        self.listeners.get_inactive_listeners(), "Inactive")

                else:
                    activeListeners = self.listeners.activeListeners
                    targetListener = [
                        l for l in activeListeners if self.args.listener in l[1]]
                ...
                if self.args.stager == 'list':
                    ...
                    for stagerName, stager in self.stagers.stagers.items():
                        print(
                            "  %s%s" %
                            ('{0: <17}'.format(stagerName),
                             stager.info['Description']))
                    print("\n")
                else:
                                ...
                                if '=' not in option:
                                    print(
                                        helpers.color(
                                            ...
                                            (option)))
                                    print(
                                        helpers.color("[!] Please use Option=Value format\n"))
                                    if self.conn:
                                        self.conn.close()
                                    sys.exit()

                                # split the passed stager options by = and set
    ...
    def database_connect(self):
        ...
        try:
            ...
            self.conn = sqlite3.connect(
                './data/empire.db', check_same_thread=False)
    ...
    def do_plugins(self, args):
        ...
        print("")
        print(helpers.color(
            "[*] Use \"plugin <plugin name>\" to load a plugin."))
    ...
    def do_plugin(self, pluginName):
        ...
        if pluginName in pluginNames:
            ...
            raise Exception(
                "[!] Error: the plugin specified does not exist in {}.".format(pluginPath))
        ...
        try:
            ...
            if parts[0] not in self.stagers.stagers:
                ...
                if not self.listeners.is_listener_valid(listener):
                    print(
                        helpers.color("[!] Please enter a valid listener name or ID"))
                else:
        ...
        if filterTerm == "":
            ...
            if len(args) == 3:
                ...
                if helpers.validate_ntlm(password):
                    # credtype, domain, username, password, host, sid="",
                    # notes=""):
                    self.credentials.add_credential(
                        "hash", domain, username, password, "")
                else:
                    self.credentials.add_credential(
                        "plaintext", domain, username, password, "")

            elif len(args) == 4:
                domain, username, password, notes = args
                if helpers.validate_ntlm(password):
                    self.credentials.add_credential(
                        "hash", domain, username, password, "", notes=notes)
                else:
                    self.credentials.add_credential(
                        "plaintext", domain, username, password, "", notes=notes)

            elif len(args) == 5:
                domain, username, password, notes, credType = args
                self.credentials.add_credential(
                    credType, domain, username, password, "", notes=notes)

            elif len(args) == 6:
                domain, username, password, notes, credType, sid = args
                self.credentials.add_credential(
                    credType, domain, username, password, "", sid=sid, notes=notes)

            else:
                print(helpers.color(
                    "[!] Format is 'add domain username password <notes> <credType> <sid>"))
                return
            ...
            try:
                ...
                if len(args) != 1:
                    print(
                        helpers.color("[!] Format is 'remove <credID>/<credID-credID>/all'"))
                else:
                    if args[0].lower() == "all":
                        choice = input(
                            helpers.color(
                                "[>] Remove all credentials from the database? [y/N] ",
                                "red"))
                        if choice.lower() != "" and choice.lower()[0] == "y":
            ...
            if len(args) != 1:
                print(
                    helpers.color("[!] Please supply an output filename/filepath."))
                return
            else:
    ...
    def do_set(self, line):
        ...
        if len(parts) == 1:
            print(
                helpers.color("[!] Please enter 'IP,IP-IP,IP/CIDR' or a file path."))
        else:
            if parts[0].lower() == "ip_whitelist":
                if parts[1] != "" and os.path.exists(parts[1]):
                    try:
                        ...
                        open_file.close()
                        self.agents.ipWhiteList = helpers.generate_ip_list(
                            ipData)
                    except Exception:
                        print(
                            helpers.color(
                                "[!] Error opening ip file %s" %
                                (parts[1])))
                else:
                    self.agents.ipWhiteList = helpers.generate_ip_list(
                        ",".join(parts[1:]))
                ...
                if parts[1] != "" and os.path.exists(parts[1]):
                    try:
                        ...
                        ipData = open_file.read()
                        open_file.close()
                        self.agents.ipBlackList = helpers.generate_ip_list(
                            ipData)
                    except Exception:
                        print(
                            helpers.color(
                ...
                if parts[1].lower() == "true":
                    ...
                    print(
                        helpers.color("[!] Valid options for obfuscate are 'true' or 'false'"))
        ...
        if parts[0].lower() == 'agents':
            ...
            if line.strip().lower() == 'stale':
                ...
                for agent in allAgents:
                    ...
                    intervalMax = (
                        agent['delay'] + agent['delay'] * agent['jitter']) + 30

                    # get the agent last check in time
                    agentTime = time.mktime(
                        time.strptime(
                            agent['lastseen_time'],
                            "%Y-%m-%d %H:%M:%S"))
                ...
                try:
                    ...
                    # grab just the agents active within the specified window
                    # (in minutes)
                    agentsToDisplay = []
                    for agent in allAgents:
                        agentTime = time.mktime(
                        ...
                        if agentTime > time.mktime(
                                time.localtime()) - (int(minutes) * 60):
                            agentsToDisplay.append(agent)
                    ...
                    print(
                        helpers.color("[!] Please enter the minute window for agent checkin."))
    ...
    def do_preobfuscate(self, line):
        ...
        if module == "" or module == "all":
            choice = input(
                helpers.color(
                    "[>] Preobfuscate all PowerShell module_source files using obfuscation command: \"" +
        ...
        # Preobfuscate a selected module_source file
            ...
            if not os.path.isfile(module_source_fullpath):
                print(
                    helpers.color(
                        "[!] The module_source file:" +
                        module_source_fullpath +
                        " does not exist."))
                return

            choice = input(
                helpers.color(
                    "[>] Preobfuscate the module_source file: " +
                    module +
                    " using obfuscation command: \"" +
                    self.obfuscateCommand +
                    "\"? [y/N] ",
                    "red"))
            if choice.lower() != "" and choice.lower()[0] == "y":
        ...
        if obfuscate_confirmation:
            ...
            for file in files:
                ...
                helpers.obfuscate_module(
                    file, self.obfuscateCommand, reobfuscate)
    ...
    def do_report(self, line):
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                'select session_id, hostname, username, checkin_time from agents')
    ...
    def complete_usemodule(self, text, line, begidx, endidx, language=None):
        ...
        for module_name in module_names:
            try:
                if self.modules.modules[module_name].info['NeedsAdmin']:
                    module_names[module_names.index(
        ...
        if language:
            module_names = [(module_name[len(language) + 1:])
                            for module_name in module_names if module_name.startswith(language)]
    ...
    def complete_preobfuscate(self, text, line, begidx, endidx):
        "Tab-complete an interact command"
        options = [(option[len('data/module_source/'):])
                   for option in helpers.get_module_source_files()]
        options.append('all')
...
class AgentsMenu(SubMenu):
    ...
    def __init__(self, mainMenu):
        ...
        # set the prompt text
        self.prompt = '(Empire: ' + helpers.color("agents",
                                                  color="blue") + ') > '
    ...
    def do_autorun(self, line):
        ...
        # read in empire commands from the specified resource file
            ...
            self.mainMenu.autoRuns[language] = self.mainMenu.buildQueue(
                resourceFile, True)
    ...
    def do_list(self, line):
        ...
        if line.lower().startswith("listeners"):
            self.mainMenu.do_list("listeners " +
                                  str(' '.join(line.split(' ')[1:])))
        elif line.lower().startswith("agents"):
            self.mainMenu.do_list("agents " +
                                  str(' '.join(line.split(' ')[1:])))
        else:
    ...
    def do_sleep(self, line):
        ...
        if len(parts) == 1:
            ...
            for agent in allAgents:
                ...
                self.mainMenu.agents.set_agent_field_db(
                    'delay', delay, sessionID)
                self.mainMenu.agents.set_agent_field_db(
                    'jitter', jitter, sessionID)
                # task the agent
                self.mainMenu.agents.add_agent_task_db(
                    sessionID, 'TASK_SHELL', 'Set-Delay ' + str(delay) + ' ' + str(jitter))

                # dispatch this event
                message = "[*] Tasked agent to delay sleep/jitter {}/{}".format(
                    delay, jitter)
                signal = json.dumps({
                    'print': True,
                    'message': message
                ...
                msg = "Tasked agent to delay sleep/jitter %s/%s" % (
                    delay, jitter)
    ...
    def do_lostlimit(self, line):
        ...
        if len(parts) == 1:
            print(helpers.color(
                "[!] Usage: 'lostlimit [agent/all] [number of missed callbacks]"))
            ...
            for agent in allAgents:
                ...
                # update this agent info in the database
                self.mainMenu.agents.set_agent_field_db(
                    'lost_limit', lostLimit, sessionID)
                # task the agent
                self.mainMenu.agents.add_agent_task_db(
                    sessionID, 'TASK_SHELL', 'Set-LostLimit ' + str(lostLimit))

                # dispatch this event
                message = "[*] Tasked agent to change lost limit {}".format(
                    lostLimit)
                signal = json.dumps({
                    'print': True,
                    'message': message
    ...
    def do_killdate(self, line):
        ...
        if len(parts) == 1:
            print(
                helpers.color("[!] Usage: 'killdate [agent/all] [01/01/2016]'"))
            ...
            for agent in allAgents:
                ...
                self.mainMenu.agents.set_agent_field_db(
                    'kill_date', date, sessionID)
                # task the agent
                self.mainMenu.agents.add_agent_task_db(
                    sessionID, 'TASK_SHELL', "Set-KillDate " + str(date))
    ...
    def do_workinghours(self, line):
        ...
        if len(parts) == 1:
            print(
                helpers.color("[!] Usage: 'workinghours [agent/all] [9:00-17:00]'"))
            ...
            for agent in allAgents:
                ...
                # update this agent's field in the database
                self.mainMenu.agents.set_agent_field_db(
                    'working_hours', hours, sessionID)
                # task the agent
                self.mainMenu.agents.add_agent_task_db(
                    sessionID, 'TASK_SHELL', "Set-WorkingHours " + str(hours))

                # dispatch this event
                message = "[*] Tasked agent to set working hours to {}".format(
                    hours)
                signal = json.dumps({
                    'print': True,
                    'message': message
    ...
    def do_remove(self, line):
        ...
        if name.lower() == 'all':
            try:
                choice = input(
                    helpers.color(
                        '[>] Remove all agents from the database? [y/N] ',
                        'red'))
                if choice.lower() != '' and choice.lower()[0] == 'y':
            ...
            # if we're removing agents that checked in longer than X minutes
            # ago
            allAgents = self.mainMenu.agents.get_agents_db()
            ...
            try:
                ...
                # grab just the agents active within the specified window (in
                # minutes)
                for agent in allAgents:
...
class AgentMenu(SubMenu):
    ...
    def __init__(self, mainMenu, sessionID):
        ...
        if agentLanguage.lower() == 'powershell':
            ...
            print(
                helpers.color(
                    "[!] Agent language %s not recognized." %
                    (agentLanguage)))
...
class PowerShellAgentMenu(SubMenu):
    ...
    def default(self, line):
        ...
        if len(parts) > 0:
            ...
            if parts[0] in self.agentCommands:
                ...
                self.mainMenu.agents.add_agent_task_db(
                    self.sessionID, "TASK_SHELL", shellcmd)
                ...
                print(
                    helpers.color("[*] Use 'help' or 'help agentcmds' to see available commands."))
    ...
    def do_list(self, line):
        ...
        if line.lower().startswith("listeners"):
            ...
            print(
                helpers.color("[!] Please use 'list [agents/listeners] <modifier>'."))
    ...
    def do_exit(self, line):
        ...
        try:
            choice = input(
                helpers.color(
                    "[>] Task agent to exit? [y/N] ",
                    "red"))
            if choice.lower() == "y":

                self.mainMenu.agents.add_agent_task_db(
                    self.sessionID, 'TASK_EXIT')
                ...
                self.mainMenu.agents.save_agent_log(
                    self.sessionID, "Tasked agent to exit")
                raise NavAgents
    ...
    def do_jobs(self, line):
        ...
        if len(parts) == 1:
            if parts[0] == '':
                ...
                # update the agent log
                self.mainMenu.agents.save_agent_log(
                    self.sessionID, "Tasked agent to get running jobs")
            else:
                print(helpers.color("[!] Please use form 'jobs kill JOB_ID'"))
        elif len(parts) == 2:
            jobID = parts[1].strip()
            self.mainMenu.agents.add_agent_task_db(
                self.sessionID, "TASK_STOPJOB", jobID)
            ...
            # update the agent log
            self.mainMenu.agents.save_agent_log(
                self.sessionID, "Tasked agent to stop job " + str(jobID))
    ...
    def do_sleep(self, line):
        ...
        if len(parts) > 0 and parts[0] != "":
            ...
            # update this agent's information in the database
            self.mainMenu.agents.set_agent_field_db(
                "delay", delay, self.sessionID)
            self.mainMenu.agents.set_agent_field_db(
                "jitter", jitter, self.sessionID)

            self.mainMenu.agents.add_agent_task_db(
                self.sessionID, "TASK_SHELL", "Set-Delay " + str(delay) + ' ' + str(jitter))
            ...
            # update the agent log
            msg = "Tasked agent to delay sleep/jitter " + \
                str(delay) + "/" + str(jitter)
            self.mainMenu.agents.save_agent_log(self.sessionID, msg)
    ...
    def do_lostlimit(self, line):
        ...
        # update this agent's information in the database
        self.mainMenu.agents.set_agent_field_db(
            "lost_limit", lostLimit, self.sessionID)
        self.mainMenu.agents.add_agent_task_db(
            self.sessionID, "TASK_SHELL", "Set-LostLimit " + str(lostLimit))
    ...
    def do_kill(self, line):
        ...
        if process == "":
            ...
            if process.isdigit():
                ...
                command = "Get-Process " + \
                    str(process) + " | %{Stop-Process $_.Id -Force}"

            self.mainMenu.agents.add_agent_task_db(
                self.sessionID, "TASK_SHELL", command)
    ...
    def do_killdate(self, line):
        ...
        if date == "":
            self.mainMenu.agents.add_agent_task_db(
                self.sessionID, "TASK_SHELL", "Get-KillDate")
            ...
            self.mainMenu.agents.save_agent_log(
                self.sessionID, "Tasked agent to get KillDate")

        else:
            # update this agent's information in the database
            self.mainMenu.agents.set_agent_field_db(
                "kill_date", date, self.sessionID)

            # task the agent
            self.mainMenu.agents.add_agent_task_db(
                self.sessionID, "TASK_SHELL", "Set-KillDate " + str(date))
    ...
    def do_workinghours(self, line):
        ...
        if hours == "":
            self.mainMenu.agents.add_agent_task_db(
                self.sessionID, "TASK_SHELL", "Get-WorkingHours")
            ...
            self.mainMenu.agents.save_agent_log(
                self.sessionID, "Tasked agent to get working hours")

        else:
            hours = hours.replace(",", "-")
            # update this agent's information in the database
            self.mainMenu.agents.set_agent_field_db(
                "working_hours", hours, self.sessionID)

            # task the agent
            self.mainMenu.agents.add_agent_task_db(
                self.sessionID, "TASK_SHELL", "Set-WorkingHours " + str(hours))
    ...
    def do_shell(self, line):
        ...
        if line != "":
            # task the agent with this shell command
            self.mainMenu.agents.add_agent_task_db(
                self.sessionID, "TASK_SHELL", "shell " + str(line))
    ...
    def do_sysinfo(self, line):
        ...
        # update the agent log
        self.mainMenu.agents.save_agent_log(
            self.sessionID, "Tasked agent to get system information")
    ...
    def do_download(self, line):
        ...
        if line != "":
            self.mainMenu.agents.add_agent_task_db(
                self.sessionID, "TASK_DOWNLOAD", line)
    ...
    def do_upload(self, line):
        ...
        if len(parts) > 0 and parts[0] != "":
            ...
            if parts[0] != "" and os.path.exists(parts[0]):
                ...
                if size > 1048576:
                    print(
                        helpers.color("[!] File size is too large. Upload limit is 1MB."))
                else:
                    # dispatch this event
                    message = "[*] Tasked agent to upload {}, {}".format(
                        uploadname, helpers.get_file_size(file_data))
                    signal = json.dumps({
                        'print': True,
                        'message': message,
                    ...
                    msg = "Tasked agent to upload %s : %s" % (
                        parts[0], hashlib.md5(file_data).hexdigest())
                    self.mainMenu.agents.save_agent_log(self.sessionID, msg)
    ...
    def do_scriptimport(self, line):
        ...
        if path != "" and os.path.exists(path):
            ...
            self.mainMenu.agents.add_agent_task_db(
                self.sessionID, "TASK_SCRIPT_IMPORT", script_data)

            # dispatch this event
            message = "[*] Tasked agent to import {}: {}".format(
                path, hashlib.md5(script_data).hexdigest())
            signal = json.dumps({
                'print': False,
                'message': message,
                'import_path': path,
            ...
            msg = "Tasked agent to import %s : %s" % (
                path, hashlib.md5(script_data).hexdigest())
            self.mainMenu.agents.save_agent_log(self.sessionID, msg)
            ...
            self.mainMenu.agents.set_agent_functions_db(
                self.sessionID, functions)
    ...
    def do_scriptcmd(self, line):
        ...
        if command != "":
            self.mainMenu.agents.add_agent_task_db(
                self.sessionID, "TASK_SCRIPT_COMMAND", command)

            # dispatch this event
            message = "[*] Tasked agent {} to run {}".format(
                self.sessionID, command)
            signal = json.dumps({
                'print': False,
                'message': message
            })
    ...
    def do_usemodule(self, line):
        ...
        if module not in self.mainMenu.modules.modules:
            ...
            module_menu = ModuleMenu(
                self.mainMenu, module, agent=self.sessionID)
            module_menu.cmdloop()
    ...
    def do_updateprofile(self, line):
        ...
        if profile != "":
            ...
            if os.path.exists(profile):
                ...
                profile = [
                    l for l in profile if not l.startswith(
                        "#" and l.strip() != "")]
                profile = profile[0]

            if not profile.strip().startswith("\"/"):
                print(helpers.color(
                    "[!] Task URIs in profiles must start with / and be enclosed in quotes!"))
            else:
                updatecmd = "Update-Profile " + profile

                # task the agent to update their profile
                self.mainMenu.agents.add_agent_task_db(
                    self.sessionID, "TASK_CMD_WAIT", updatecmd)

                # dispatch this event
                message = "[*] Tasked agent to update profile {}".format(
                    profile)
                signal = json.dumps({
                    'print': False,
                    'message': message
        ...
        if line:
            ...
            if not self.mainMenu.listeners.is_listener_valid(listenerID):
                ...
                if activeListener['moduleName'] != 'meterpreter' or activeListener['moduleName'] != 'http_mapi':
                    ...
                    listenerComms = self.mainMenu.listeners.loadedListeners[activeListener['moduleName']].generate_comms(
                        listenerOptions, language="powershell")

                    self.mainMenu.agents.add_agent_task_db(
                        self.sessionID, "TASK_UPDATE_LISTENERNAME", listenerOptions['Name']['Value'])
                    self.mainMenu.agents.add_agent_task_db(
                        self.sessionID, "TASK_SWITCH_LISTENER", listenerComms)
                    ...
                    print(
                        helpers.color(
                            "[!] Ineligible listener for updatecomms command: %s" %
    ...
    def do_psinject(self, line):
        ...
        if line:
            ...
            if self.mainMenu.modules.modules['powershell/management/psinject']:
                ...
                if listenerID != '' and self.mainMenu.listeners.is_listener_valid(
                        listenerID):
                    if len(line.split(' ')) == 2:
                    ...
                    module.options['Agent']['Value'] = self.mainMenu.agents.get_agent_name_db(
                        self.sessionID)
                    module_menu = ModuleMenu(
                        self.mainMenu, 'powershell/management/psinject')
                    module_menu.do_execute("")

                else:
                    print(
                        helpers.color("[!] Please enter <listenerName> <pid>"))

            else:
                print(
                    helpers.color("[!] powershell/management/psinject module not loaded"))

        else:
            print(
                helpers.color("[!] Injection requires you to specify listener"))
                ...
                if listenerID != '' and self.mainMenu.listeners.is_listener_valid(
                    ...
                    if len(line.split(' ')) == 3:
                        ...
                        if target.isdigit():
                            ...
                            print(
                                helpers.color('[!] Please enter a valid process ID.'))

                    module.options['Agent']['Value'] = self.mainMenu.agents.get_agent_name_db(
                        self.sessionID)
                    module_menu = ModuleMenu(
                        self.mainMenu, 'powershell/management/shinject')
                    module_menu.do_execute("")
                else:
        ...
        if line:
            ...
            if self.mainMenu.modules.modules['powershell/code_execution/invoke_shellcode']:
                ...
                if listenerID != '' and self.mainMenu.listeners.is_listener_valid(
                    ...
                    module_menu = ModuleMenu(
                        self.mainMenu, 'powershell/code_execution/invoke_shellcode')
                    module_menu.cmdloop()
                ...
                print(helpers.color(
                    "[!] powershell/code_execution/invoke_shellcode module not loaded"))
    ...
    def do_sc(self, line):
        ...
        if len(line.strip()) > 0:
            ...
            try:
                ...
                print(
                    helpers.color("[*] JPEG Ratio incorrect. Has been set to 80."))
                screenshot_ratio = "80"
        else:
            screenshot_ratio = ''
        ...
        if self.mainMenu.modules.modules['powershell/collection/screenshot']:
            ...
            module_menu = ModuleMenu(
                self.mainMenu, 'powershell/collection/screenshot')
            module_menu.do_execute("")

        else:
            print(
                helpers.color("[!] powershell/collection/screenshot module not loaded"))
        ...
        if line:
            ...
            if listenerID != '' and self.mainMenu.listeners.is_listener_valid(
                ...
                if self.mainMenu.modules.modules['powershell/management/spawn']:
                    ...
                    module_menu = ModuleMenu(
                        self.mainMenu, "powershell/management/spawn")
                    module_menu.cmdloop()

                else:
                    print(
                        helpers.color("[!] management/spawn module not loaded"))
        ...
        if line:
            ...
            if listenerID != '' and self.mainMenu.listeners.is_listener_valid(
                ...
                if self.mainMenu.modules.modules['powershell/privesc/bypassuac_eventvwr']:
                    ...
                    # jump to the spawn module
                    module_menu = ModuleMenu(
                        self.mainMenu, 'powershell/privesc/bypassuac_eventvwr')
                    module_menu.do_execute('')

                else:
                    print(
                        helpers.color("[!] powershell/privesc/bypassuac_eventvwr module not loaded"))
    ...
    def do_pth(self, line):
        ...
        if self.mainMenu.modules.modules['powershell/credentials/mimikatz/pth']:
            ...
            module = self.mainMenu.modules.reload_module(
            ...
            module_menu = ModuleMenu(
                self.mainMenu, 'powershell/credentials/mimikatz/pth')
            module_menu.do_execute('')
    ...
    def do_steal_token(self, line):
        ...
        if self.mainMenu.modules.modules['powershell/credentials/tokens']:
            ...
            module_menu = ModuleMenu(
                self.mainMenu, 'powershell/credentials/tokens')
            module_menu.do_execute('')
    ...
    def complete_psinject(self, text, line, begidx, endidx):
        ...
        mline = line.partition(' ')[2]
        offs = len(mline) - len(text)
        return [s[offs:] for s in self.mainMenu.listeners.get_listener_names()
    ...
    def complete_usemodule(self, text, line, begidx, endidx):
        "Tab-complete an Empire PowerShell module path"
        return self.mainMenu.complete_usemodule(
            text, line, begidx, endidx, language='powershell')
...
class PythonAgentMenu(SubMenu):
    ...
    def do_cd(self, line):
        ...
        if line != "":
            ...
            # dispatch this event
            message = "[*] Tasked agent to change active directory to {}".format(
                line)
            signal = json.dumps({
                'print': False,
                'message': message
    ...
    def do_sleep(self, line):
        ...
        if len(parts) >= 1:
            try:
                ...
                print(
                    helpers.color("[!] Please only enter integer for 'interval'"))
                return
        ...
        if len(parts) > 1:
            try:
                ...
                print(
                    helpers.color("[!] Please only enter integer for '[jitter]'"))
                return
        ...
        if delay == "":
            ...
            self.mainMenu.agents.save_agent_log(
                self.sessionID, "Tasked agent to display delay/jitter")
    ...
    def do_lostlimit(self, line):
        ...
        if lostLimit == "":
            ...
            self.mainMenu.agents.save_agent_log(
                self.sessionID, "Tasked agent to display lost limit")
        else:
            # update this agent's information in the database
            self.mainMenu.agents.set_agent_field_db(
                "lost_limit", lostLimit, self.sessionID)
    ...
    def do_killdate(self, line):
        ...
        if killDate == "":
            ...
            self.mainMenu.agents.save_agent_log(
                self.sessionID, "Tasked agent to display killDate")
        else:
            # update this agent's information in the database
            self.mainMenu.agents.set_agent_field_db(
                "kill_date", killDate, self.sessionID)
    ...
    def do_workinghours(self, line):
        ...
        if hours == "":
            ...
            self.mainMenu.agents.add_agent_task_db(
                self.sessionID,
                "TASK_CMD_WAIT",
                "global workingHours; workingHours= '%s'" %
                (hours))
    ...
    def do_shell(self, line):
        ...
        if line != "":
            # task the agent with this shell command
            self.mainMenu.agents.add_agent_task_db(
                self.sessionID, "TASK_SHELL", str(line))
    ...
    def do_python(self, line):
        ...
        if line != "":
            # task the agent with this shell command
            self.mainMenu.agents.add_agent_task_db(
                self.sessionID, "TASK_CMD_WAIT", str(line))
    ...
    def do_pythonscript(self, line):
        ...
        if os.path.splitext(path)[-1] == '.py' and os.path.isfile(path):
            ...
            self.mainMenu.agents.add_agent_task_db(
                self.sessionID, "TASK_SCRIPT_COMMAND", encScript)

            # dispatch this event
            message = "[*] Tasked agent to execute Python script: {}".format(
                filename)
            signal = json.dumps({
                'print': True,
                'message': message,
        ...
        if len(parts) > 0 and parts[0] != "":
            ...
            if parts[0] != "" and os.path.exists(parts[0]):
                ...
                if size > 1048576:
                    ...
                    print(
                        helpers.color(
                            "[*] Original tasked size of %s for upload: %s" %
                            (uploadname, helpers.get_file_size(fileData)), color="green"))

                    original_md5 = hashlib.md5(fileData).hexdigest()
                    # update the agent log with the filename and MD5
                    msg = "Tasked agent to upload " + \
                        parts[0] + " : " + original_md5
                    self.mainMenu.agents.save_agent_log(self.sessionID, msg)
                    ...
                    message = "[*] Starting upload of {}, final size {}".format(
                        uploadname, helpers.get_file_size(fileData))
                    signal = json.dumps({
                        'print': True,
                        'message': message,
    ...
    def do_osx_screenshot(self, line):
        ...
        if self.mainMenu.modules.modules['python/collection/osx/native_screenshot']:
            ...
            module_menu = ModuleMenu(
                self.mainMenu, 'python/collection/osx/native_screenshot')
            ...
            print(
                helpers.color("[!] python/collection/osx/screenshot module not loaded"))
...
""" % (line)
            # task the agent with this shell command
            self.mainMenu.agents.add_agent_task_db(
                self.sessionID, "TASK_CMD_WAIT", str(cmd))
    ...
    def do_loadpymodule(self, line):
        ...
        if os.path.splitext(path)[-1] == '.zip' and os.path.isfile(path):
            # open a handle to the file and save the data to a variable, zlib
            # compress
            ...
            # dispatch this event
            message = "[*] Tasked agent to import {}, md5: {}".format(
                path, hashlib.md5(module_data).hexdigest())
            signal = json.dumps({
                'print': True,
                'message': message,
                'import_path': path,
            ...
            msg = "Tasked agent to import " + path + \
                " : " + hashlib.md5(module_data).hexdigest()
            self.mainMenu.agents.save_agent_log(self.sessionID, msg)
            ...
            self.mainMenu.agents.add_agent_task_db(
                self.sessionID, "TASK_IMPORT_MODULE", data)
        else:
            print(
                helpers.color(
                    "[!] Please provide a valid zipfile path",
                    color="red"))
    ...
    def do_viewrepo(self, line):
        ...
        self.mainMenu.agents.add_agent_task_db(
            self.sessionID, "TASK_VIEW_MODULE", repoName)
    ...
    def do_removerepo(self, line):
        ...
        self.mainMenu.agents.save_agent_log(self.sessionID, msg)
        self.mainMenu.agents.add_agent_task_db(
            self.sessionID, "TASK_REMOVE_MODULE", repoName)
    ...
    def complete_usemodule(self, text, line, begidx, endidx):
        ...
        return self.mainMenu.complete_usemodule(
            text, line, begidx, endidx, language='python')
...
class ListenersMenu(SubMenu):
    ...
    def do_info(self, line):
        ...
        if listenerName not in self.mainMenu.listeners.activeListeners:
            ...
            messages.display_active_listener(
                self.mainMenu.listeners.activeListeners[listenerName])
    ...
    def do_launcher(self, line):
        ...
        if len(parts) != 2:
            print(
                helpers.color("[!] Please enter 'launcher <language> <listenerName>'"))
            return
        else:
    ...
    def do_edit(self, line):
        ...
        if len(arguments) < 2:
            print(helpers.color(
                "[!] edit <listener name> <option name> <option value> (leave value blank to unset)"))
            return
        if len(arguments) == 2:
            arguments.append("")
        self.mainMenu.listeners.update_listener_options(
            arguments[0], arguments[1], arguments[2])
        if arguments[0] in list(
...
class ListenerMenu(SubMenu):
    ...
    def __init__(self, mainMenu, listenerName):
        ...
        if listenerName not in self.mainMenu.listeners.loadedListeners:
            print(
                helpers.color(
                    "[!] Listener '%s' not currently valid!" %
                    (listenerName)))
            raise NavListeners()
        ...
        # set the text prompt
        self.prompt = '(Empire: ' + helpers.color("listeners/%s" %
                                                  (listenerName), 'red') + ') > '
    ...
    def do_execute(self, line):
        ...
        self.mainMenu.listeners.start_listener(
            self.listenerName, self.listener)
    ...
    def do_set(self, line):
        ...
        try:
            ...
            if option not in self.listener.options:
                ...
                if self.listener.options[option]['Description'].startswith(
                        "Switch."):
                ...
                self.mainMenu.listeners.set_listener_option(
                    self.listenerName, option, value)

        except Exception as e:
            print(
                helpers.color(
                    "[!] Error in setting listener option: %s" %
                    (e)))
...
class ModuleMenu(SubMenu):
    ...
    def __init__(self, mainMenu, moduleName, agent=None):
        ...
        try:
            ...
            # set the prompt text
            self.prompt = '(Empire: ' + \
                helpers.color(self.moduleName, color="blue") + ') > '
    ...
    def validate_options(self, prompt):
        ...
        for option, values in self.module.options.items():
            if values['Required'] and (
                (not values['Value']) or (
                    values['Value'] == '')):
                print(
                    helpers.color("[!] Error: Required module option missing."))
                return False
        ...
        if 'Agent' in self.module.options:
            ...
            try:
                # if we're running this module for all agents, skip this
                # validation
                if sessionID.lower() != "all" and sessionID.lower() != "autorun":
                    moduleLangVersion = float(
                        self.module.info['MinLanguageVersion'])
                    agentLangVersion = float(
                        self.mainMenu.agents.get_language_version_db(sessionID))

                    # check if the agent/module PowerShell versions are
                    # compatible
                    if moduleLangVersion > agentLangVersion:
                        print(
                            helpers.color(
                                "[!] Error: module requires language version %s but agent running version %s" %
                                (moduleLangVersion, agentPSVersion)))
                        return False
            except Exception as e:
                print(
                    helpers.color(
                        "[!] Invalid module or agent language version: %s" %
                        (e)))
                return False
            ...
            if self.module.info['NeedsAdmin']:
                ...
                if sessionID.lower() != "all" and sessionID.lower() != "autorun":
                    if not self.mainMenu.agents.is_agent_elevated(sessionID):
                        print(
                            helpers.color("[!] Error: module needs to run in an elevated context."))
                        return False

        # if the module isn't opsec safe, prompt before running (unless
        # "execute noprompt" was issued)
        if prompt and (
                'OpsecSafe' in self.module.info) and (
                not self.module.info['OpsecSafe']):

            try:
                choice = input(
                    helpers.color(
                        "[>] Module is not opsec safe, run? [y/N] ",
                        "red"))
    ...
    def do_set(self, line):
        ...
        try:
            ...
            if option not in self.module.options:
                ...
                # "set OPTION"
                # check if we're setting a switch
                if self.module.options[option]['Description'].startswith(
                        "Switch."):
            ...
            print(
                helpers.color("[!] Error in setting option, likely invalid option name."))
    ...
    def do_execute(self, line):
        ...
        if self.moduleName.lower().startswith('external/'):
            ...
            moduleData = self.module.generate(
                self.mainMenu.obfuscate, self.mainMenu.obfuscateCommand)

            if not moduleData or moduleData == "":
                print(
                    helpers.color("[!] Error: module produced an empty script"))
            try:
                moduleData.decode('ascii')
            except UnicodeDecodeError:
                print(
                    helpers.color("[!] Error: module source contains non-ascii characters"))
                return
            ...
            if str(self.module.info['Background']).lower() == "true":
                ...
                if extention and extention != "":
                    ...
                    moduleData = saveFilePrefix.rjust(
                        15) + extention.rjust(5) + moduleData
            ...
            if agentName.lower() == "all":
                try:
                    choice = input(
                        helpers.color(
                            "[>] Run module on all agents? [y/N] ",
                            "red"))
                    if choice.lower() != "" and choice.lower()[0] == "y":

                        # signal everyone with what we're doing
                        message = "[*] Tasking all agents to run {}".format(
                            self.moduleName)
                        signal = json.dumps({
                            'print': True,
                            'message': message
                        ...
                        dispatcher.send(
                            signal, sender="agents/all/{}".format(self.moduleName))
                        ...
                        for agent in self.mainMenu.agents.get_agents_db():
                            ...
                            self.mainMenu.agents.add_agent_task_db(
                                sessionID, taskCommand, moduleData)

                            # update the agent log
                            # dispatcher.send("[*] Tasked agent "+sessionID+" to run module " + self.moduleName, sender="Empire")
                            message = "[*] Tasked agent {} to run module {}".format(
                                sessionID, self.moduleName)
                            signal = json.dumps({
                                'print': True,
                                'message': message,
                            ...
                            dispatcher.send(
                                signal, sender="agents/{}/{}".format(sessionID, self.moduleName))
                            msg = "Tasked agent to run module {}".format(
                                self.moduleName)
                            self.mainMenu.agents.save_agent_log(sessionID, msg)
            ...
            # set the script to be the global autorun
                ...
                message = "[*] Set module {} to be global script autorun.".format(
                    self.moduleName)
                signal = json.dumps({
                    'print': True,
                    'message': message
                ...
                if not self.mainMenu.agents.is_agent_present(agentName):
                    ...
                    # set the agent's tasking in the cache
                    self.mainMenu.agents.add_agent_task_db(
                        agentName, taskCommand, moduleData)

                    # update the agent log
                    message = "[*] Tasked agent {} to run module {}".format(
                        agentName, self.moduleName)
                    signal = json.dumps({
                        'print': True,
                        'message': message,
                        'options': self.module.options
                    ...
                    dispatcher.send(
                        signal, sender="agents/{}/{}".format(agentName, self.moduleName))
    ...
    def complete_set(self, text, line, begidx, endidx):
        ...
        if line.split(' ')[1].lower() == "agent":
            ...
            agentNames = self.mainMenu.agents.get_agent_names_db() + \
                ["all", "autorun"]
            end_line = ' '.join(line.split(' ')[1:])
...
class StagerMenu(SubMenu):
    ...
    def __init__(self, mainMenu, stagerName, listener=None):
        ...
        # set the prompt text
        self.prompt = '(Empire: ' + helpers.color("stager/" + \
                        self.stagerName, color="blue") + ') > '
        ...
        for option, values in self.stager.options.items():
            if values['Required'] and (
                (not values['Value']) or (
                    values['Value'] == '')):
                print(
                    helpers.color("[!] Error: Required stager option missing."))
                return False
    ...
    def do_set(self, line):
        ...
        try:
            ...
            if option not in self.stager.options:
                ...
                # "set OPTION"
                # check if we're setting a switch
                if self.stager.options[option]['Description'].startswith(
                        "Switch."):
